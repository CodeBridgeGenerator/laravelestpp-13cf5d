...
client.service("customerTypes")
                        .find({ query: { $select: ["_id","type"] } })
                        .then((data) => {
                        setcustomerTypeIds(data.data);
                        });

    }, [props.show]);

    const onRun = async () => {
        let fakeData = customersFakerFactory(props.user,fakerCount,userIdIds,branchIdIds,customerTypeIds);
